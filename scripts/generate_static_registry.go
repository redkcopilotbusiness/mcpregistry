...
func main() {
  ...
  for _, s := range servers {
    ...
    out.Servers = append(out.Servers, ServerResponse{
      ...
      Meta: ResponseMeta{Official: OfficialMeta{
        Status: "active",
        PublishedAt: now, // For simplicity we set both to now; could preserve previous publishedAt if needed
        UpdatedAt: now,
        IsLatest: true,
        VersionSequence: seq,
      }},
  ...
  if err := os.WriteFile(existingIndexPath, jsonBytes, 0o644); err != nil {
    fmt.Fprintf(os.Stderr, "failed to write index.json: %v\n", err)
    os.Exit(1)
  }

  fmt.Println("Generated docs/v0/servers/index.json with versionSequence numbers.")
}
